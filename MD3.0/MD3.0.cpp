...
void F1(double x1, double x2, int m1, int m2, int m3, int m4, std::string file_name) {
	...
	for (int iter = 0; iter < m3; iter++) {
		...
		for (int j = 0; j < particles_count; j++) {
			fprintf(debug_file2, "%d %.5le %.5le", j, abs(middle_y1[j] - middle_y2[j]), abs(middle_z1[j] - middle_z2[j]));
			...
			fprintf(debug_file2, "%d %.5le %.5le", j, abs(y_max - y_min), abs(z_max - z_min));
		}
		fclose(debug_file2);
