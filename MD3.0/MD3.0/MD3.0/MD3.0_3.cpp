...
#include <time.h>    // библиотека для подстчћта времени
...
// Количество ячеек по Y, Z и X(K2), на которые разбивается объћм.
// Количество ячеек расчитывается динамически при загрузке системы
// или при посеве (см. функцию load).
long int K, K2;

// число частиц во всћм объћме, переопределяется в функциях load и new_seed
long int NP;

// глобальный счћтчик столкновений в системе
...
// параметры объћма, задаются в load()
double A, dA, L, dL;

// Глобальная переменная для подсчћта общей кинетической энергии всех частиц
double global_E = 0.0;
...
// клетка. Объћм системы разделћн на множество клеток,
...
// массив клеток для всего объћма
...
A - размер системы (области объћма, где может находиться центр частицы) по Y и Z.
L
...
int check_particles() {
	...
	for (long int i = 0; i < NP; i++) {
		...
		// проверяем что частицы находятся в объћме
		...
		/*
		// Проверяем на правильное ли событие в линейке времћн ссылается частица
...
/*
Функция подъћма элемента по очереди событий к началу очереди
...
Аргументы:
i - частица, с участием которой произойдћт новое событие
j - номер частицы с которой столкнћтся частица i или номер события
...
void add_event(long int &i, long int &j, long int &vp1, long int &vp2) {
	/*
	Рассчитываем полное время нового события от начала отсчћта
	...
	*/
	...
	/*
	Находим позицию в дереве времћн для нового события.
	Изначально помещаем это событие вниз дерева и позволяем
	ему подняться по дереву, если данное событие произойдћт раньше чем
	другие события
	*/
	...
	необходимо для второй частицы сохранить данные о ећ новом событии
	*/
	if (j >= 0) {
	...
	// записываем новое событие в выбранную ячейку в дереве времћн
...
void clear_particle_events(long int &i) {
	...
	if (f > 0) {
		...
		if (time_queue[f].im == i) {
			...
			места предполагаемого столкновения и после этого для нећ
...
/*
Функция расчћта ближайшего события для частицы
...
void retime(long int &i) {
	...
	double dt, dt_min = 1.0e+20;  // переменные для рассчћта времени для ближайшего события
							...
							*/
							...
							if (bij < 0.0) {
								...
								if (d > 0.0) {
									...
									в результате получаем время dt через которое это соударение произойдћт для
	...
	необходимо перезаписать ећ предыдущее ближайшее событие и синхронизовать
	...
	*/
	...
	/*
	В случае если при расчћте ближайшего события мы получили отрицательное время
...
Загружаются параметры объћма, координаты и скорости всех частиц.
...
void load_seed(std::string file_name) {
	...
	*/
	...
	dL = L / (K2 - 1);      // выбранные так, чтобы, ячейки совпадали
							...
							использоваться для рассчћта времени до пересечения границ
...
Аргументы:
NN - число частиц в ребре объћмо центрированного кристалла, на основе
...
void clear_cell_from_virt_particle(long i, long j) {
	...
	if (p1_box.particles[p1.box_i[j]] % NP == i) {
		...
		if (p1.box_i[j] <= end) {
			...
			вместо нећ вставляем частицу из конца списка частиц в ячейке и
			...
			for (int j = 0; j < 4; ++j) {
				if (p1.x[j] > 0.0) {
					...
					*/
					...
					/*
					После того, как индекс ячейки для частицы был изменћн,
					добавляем частицу в новую ячейку (или возвращаем ећ в старую
...
производит рассчћт динамики системы в течении 1 соударения
		...
		*/
		...
		for (long i = 0; i < NP; ++i) {
			...
			*/
			...
			после синхронизации ећ по времени с глобальным временем системы
			*/
...
void profile_medium(double x1, double x2, int dots_per_particle, long steps, std::string file_name) {
	...
	for (j = 0; j < NP; j++) {
		...
		временем системы и проверяем ећ координату Х,
		если частица находится в указанном диапазоне,
		то сохраняем ећ Y и Z координаты в файл.
		*/
...
void F1(double x1, double x2, int m1, int m2, int m3, int m4, std::string file_name) {
	...
	// Динамически создаћм двумерные массивы (это необходимо делать динамически,
...
delta_max - максимальный разрешћнный шаг по L
...
void compress(double compress_to_etta, double delta_max, long int KK1, long int KK2, long int KK3, int type) {
	...
	// задаћм плотность с точностью в 12 знаков
		...
		if (type < 3) {
			if (etta < compress_to_etta) {
				...
				dx = deltaL / 2.0;  // рассчитываем смещение для всех частиц
									...
									// Если следущее смещение стенки сожмћт систему больше
				...
				//Если систему необходимо расширить, то просто берћм
				//наибольший разрешћнный шаг для изменения коорднаты стенки
