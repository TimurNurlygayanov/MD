...
#include <time.h>    // áèáëèîòåêà äëÿ ïîäñò÷žòà âðåìåíè
...
// Êîëè÷åñòâî ÿ÷ååê ïî Y, Z è X(K2), íà êîòîðûå ðàçáèâàåòñÿ îáúžì.
// Êîëè÷åñòâî ÿ÷ååê ðàñ÷èòûâàåòñÿ äèíàìè÷åñêè ïðè çàãðóçêå ñèñòåìû
// èëè ïðè ïîñåâå (ñì. ôóíêöèþ load).
long int K, K2;

// ÷èñëî ÷àñòèö âî âñžì îáúžìå, ïåðåîïðåäåëÿåòñÿ â ôóíêöèÿõ load è new_seed
long int NP;

// ãëîáàëüíûé ñ÷žò÷èê ñòîëêíîâåíèé â ñèñòåìå
...
// ïàðàìåòðû îáúžìà, çàäàþòñÿ â load()
double A, dA, L, dL;

// Ãëîáàëüíàÿ ïåðåìåííàÿ äëÿ ïîäñ÷žòà îáùåé êèíåòè÷åñêîé ýíåðãèè âñåõ ÷àñòèö
double global_E = 0.0;
...
// êëåòêà. Îáúžì ñèñòåìû ðàçäåëží íà ìíîæåñòâî êëåòîê,
...
// ìàññèâ êëåòîê äëÿ âñåãî îáúžìà
...
A - ðàçìåð ñèñòåìû (îáëàñòè îáúžìà, ãäå ìîæåò íàõîäèòüñÿ öåíòð ÷àñòèöû) ïî Y è Z.
L
...
int check_particles() {
	...
	for (long int i = 0; i < NP; i++) {
		...
		// ïðîâåðÿåì ÷òî ÷àñòèöû íàõîäÿòñÿ â îáúžìå
		...
		/*
		// Ïðîâåðÿåì íà ïðàâèëüíîå ëè ñîáûòèå â ëèíåéêå âðåìží ññûëàåòñÿ ÷àñòèöà
...
/*
Ôóíêöèÿ ïîäúžìà ýëåìåíòà ïî î÷åðåäè ñîáûòèé ê íà÷àëó î÷åðåäè
...
Àðãóìåíòû:
i - ÷àñòèöà, ñ ó÷àñòèåì êîòîðîé ïðîèçîéäžò íîâîå ñîáûòèå
j - íîìåð ÷àñòèöû ñ êîòîðîé ñòîëêížòñÿ ÷àñòèöà i èëè íîìåð ñîáûòèÿ
...
void add_event(long int &i, long int &j, long int &vp1, long int &vp2) {
	/*
	Ðàññ÷èòûâàåì ïîëíîå âðåìÿ íîâîãî ñîáûòèÿ îò íà÷àëà îòñ÷žòà
	...
	*/
	...
	/*
	Íàõîäèì ïîçèöèþ â äåðåâå âðåìží äëÿ íîâîãî ñîáûòèÿ.
	Èçíà÷àëüíî ïîìåùàåì ýòî ñîáûòèå âíèç äåðåâà è ïîçâîëÿåì
	åìó ïîäíÿòüñÿ ïî äåðåâó, åñëè äàííîå ñîáûòèå ïðîèçîéäžò ðàíüøå ÷åì
	äðóãèå ñîáûòèÿ
	*/
	...
	íåîáõîäèìî äëÿ âòîðîé ÷àñòèöû ñîõðàíèòü äàííûå î åž íîâîì ñîáûòèè
	*/
	if (j >= 0) {
	...
	// çàïèñûâàåì íîâîå ñîáûòèå â âûáðàííóþ ÿ÷åéêó â äåðåâå âðåìží
...
void clear_particle_events(long int &i) {
	...
	if (f > 0) {
		...
		if (time_queue[f].im == i) {
			...
			ìåñòà ïðåäïîëàãàåìîãî ñòîëêíîâåíèÿ è ïîñëå ýòîãî äëÿ íåž
...
/*
Ôóíêöèÿ ðàñ÷žòà áëèæàéøåãî ñîáûòèÿ äëÿ ÷àñòèöû
...
void retime(long int &i) {
	...
	double dt, dt_min = 1.0e+20;  // ïåðåìåííûå äëÿ ðàññ÷žòà âðåìåíè äëÿ áëèæàéøåãî ñîáûòèÿ
							...
							*/
							...
							if (bij < 0.0) {
								...
								if (d > 0.0) {
									...
									â ðåçóëüòàòå ïîëó÷àåì âðåìÿ dt ÷åðåç êîòîðîå ýòî ñîóäàðåíèå ïðîèçîéäžò äëÿ
	...
	íåîáõîäèìî ïåðåçàïèñàòü åž ïðåäûäóùåå áëèæàéøåå ñîáûòèå è ñèíõðîíèçîâàòü
	...
	*/
	...
	/*
	Â ñëó÷àå åñëè ïðè ðàñ÷žòå áëèæàéøåãî ñîáûòèÿ ìû ïîëó÷èëè îòðèöàòåëüíîå âðåìÿ
...
Çàãðóæàþòñÿ ïàðàìåòðû îáúžìà, êîîðäèíàòû è ñêîðîñòè âñåõ ÷àñòèö.
...
void load_seed(std::string file_name) {
	...
	*/
	...
	dL = L / (K2 - 1);      // âûáðàííûå òàê, ÷òîáû, ÿ÷åéêè ñîâïàäàëè
							...
							èñïîëüçîâàòüñÿ äëÿ ðàññ÷žòà âðåìåíè äî ïåðåñå÷åíèÿ ãðàíèö
...
Àðãóìåíòû:
NN - ÷èñëî ÷àñòèö â ðåáðå îáúžìî öåíòðèðîâàííîãî êðèñòàëëà, íà îñíîâå
...
void clear_cell_from_virt_particle(long i, long j) {
	...
	if (p1_box.particles[p1.box_i[j]] % NP == i) {
		...
		if (p1.box_i[j] <= end) {
			...
			âìåñòî íåž âñòàâëÿåì ÷àñòèöó èç êîíöà ñïèñêà ÷àñòèö â ÿ÷åéêå è
			...
			for (int j = 0; j < 4; ++j) {
				if (p1.x[j] > 0.0) {
					...
					*/
					...
					/*
					Ïîñëå òîãî, êàê èíäåêñ ÿ÷åéêè äëÿ ÷àñòèöû áûë èçìåíží,
					äîáàâëÿåì ÷àñòèöó â íîâóþ ÿ÷åéêó (èëè âîçâðàùàåì åž â ñòàðóþ
...
ïðîèçâîäèò ðàññ÷žò äèíàìèêè ñèñòåìû â òå÷åíèè 1 ñîóäàðåíèÿ
		...
		*/
		...
		for (long i = 0; i < NP; ++i) {
			...
			*/
			...
			ïîñëå ñèíõðîíèçàöèè åž ïî âðåìåíè ñ ãëîáàëüíûì âðåìåíåì ñèñòåìû
			*/
...
void profile_medium(double x1, double x2, int dots_per_particle, long steps, std::string file_name) {
	...
	for (j = 0; j < NP; j++) {
		...
		âðåìåíåì ñèñòåìû è ïðîâåðÿåì åž êîîðäèíàòó Õ,
		åñëè ÷àñòèöà íàõîäèòñÿ â óêàçàííîì äèàïàçîíå,
		òî ñîõðàíÿåì åž Y è Z êîîðäèíàòû â ôàéë.
		*/
...
void F1(double x1, double x2, int m1, int m2, int m3, int m4, std::string file_name) {
	...
	// Äèíàìè÷åñêè ñîçäàžì äâóìåðíûå ìàññèâû (ýòî íåîáõîäèìî äåëàòü äèíàìè÷åñêè,
...
delta_max - ìàêñèìàëüíûé ðàçðåøžííûé øàã ïî L
...
void compress(double compress_to_etta, double delta_max, long int KK1, long int KK2, long int KK3, int type) {
	...
	// çàäàžì ïëîòíîñòü ñ òî÷íîñòüþ â 12 çíàêîâ
		...
		if (type < 3) {
			if (etta < compress_to_etta) {
				...
				dx = deltaL / 2.0;  // ðàññ÷èòûâàåì ñìåùåíèå äëÿ âñåõ ÷àñòèö
									...
									// Åñëè ñëåäóùåå ñìåùåíèå ñòåíêè ñîæìžò ñèñòåìó áîëüøå
				...
				//Åñëè ñèñòåìó íåîáõîäèìî ðàñøèðèòü, òî ïðîñòî áåðžì
				//íàèáîëüøèé ðàçðåøžííûé øàã äëÿ èçìåíåíèÿ êîîðäíàòû ñòåíêè
